...
// distinct reticles for red-dot, reflex, holographic, and mid-scope
// sights.
//
// Reticle visuals are uniform regardless of attachment rarity — every
// sight is drawn at full visibility so the player always reads the
// reticle clearly. Rarity instead scales the attachment's mechanical
// modifier values (see attachments.js _rollAttachmentRarity).
...
// for visibility on bright backgrounds. Stroke widths picked for
// clear readability; identical regardless of attachment rarity.
function svgRedDot() {
  const dotR = 2.4;
  const haloR = 6.5;
  return `<g>
...
// styled after a Russian PK-A reflex.
function svgReflex() {
  const sw = 1.5;
  return `<g fill='none' stroke-linecap='round'>
...
// Public lookup. Resolves an SVG cursor URL string for a sight id.
// Falls back to the default tactical crosshair.
function _cursorFor(sightId) {
  const inner = (() => {
    switch (sightId) {
      case 'sight_reddot': return svgRedDot();
...
// weapon.attachments.topRail (the equipped sight) and swaps to the
// matching reticle. Cached by sight id so repeated swaps to the
// same sight don't reapply the same data URL string.
let _appliedKey = '';
export function setCursorForWeapon(weapon) {
  const sight = weapon?.attachments?.topRail;
  const sightId = sight?.id || sight?.kind || '';
  if (sightId === _appliedKey) return;
  _appliedKey = sightId;
  document.body.style.cursor = _cursorFor(sightId);
}
