...
export class GunmanManager {
  ...
  _updateRanged(g, ctx, dt) {
    ...
    if (canSee) {
      ...
      // Pass 2 ledge-mantle on patrol — when this enemy is on a ledge
      // already, run the dwell timer and drop off when it expires.
      // Otherwise, on each fresh patrol leg, 30% chance to try a mantle.
      if (g._mountedLedge) {
        g._ledgeDwellT = (g._ledgeDwellT || 0) - dt;
        if (g._ledgeDwellT <= 0) {
          // Drop off back inside the room. Inline (see mantle note).
      ...
      g.patrolT -= dt;
      if (g.patrolT <= 0 && !g._mountedLedge) {
        ...
        // Pass 2 — 30% chance per patrol leg to try a ledge mantle.
        // Skipped if another gunman already occupies the candidate.
        if (Math.random() < 0.30 && ctx.level && ctx.level.ledges
            && ctx.level.ledges.length) {
          try {
            const facing = g.group.rotation.y;
            const found = mantleableAt(ctx.level, g.group.position, facing);
            if (found && found.ledge) {
              const taken = g.manager?.gunmen?.some(
                o => o !== g && o._mountedLedge === found.ledge);
              if (!taken) {
                // ledges.applyMantle expects player.position or
                // player.mesh.position; gunmen carry .group instead.
                // Inline the lift here so we don't need to muck with
                // the ledges API.
