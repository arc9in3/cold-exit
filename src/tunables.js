...
export const tunables = {
  ...
  status: {
    // Statuses are punchier: half the duration of the long-tail tuning
    // and +75% DPS so the player feels the pressure to heal instead of
    // ignoring a tick they can outregen. Edge-only red vignette keeps
    // the visual reminder without oppressing mid-screen.
    bleedDps: 1.75,         // current-HP DoT while bleeding
    brokenDps: 1.17,        // current-HP DoT while broken
    brokenCapDps: 0.7,      // regenCap DoT while broken
    bleedDuration: 18,      // seconds a bleed lasts untreated
    brokenDuration: 30,     // seconds a broken bone lasts untreated
  ...
  weapons: [
    ...
    },
    {
      ...
      // User feedback: feel is good, just sustains too long.
      //   magSize 100 → 87 (87/20 = 4.35s held fire vs 5s, +15%
      //                     ammo cost per second of fire).
      //   reloadTime 4.0 → 6.0s — longer commit window between
      //                          tank-swaps.
      // tickRate (and thus DPS) unchanged on purpose; the moment-
      // to-moment feel they liked stays intact.
      magSize: 87,
      reloadTime: 6.0,
      flameAngleDeg: 36,
      flameTickRate: 20,
    ...
    },
    {
      ...
      rarity: 'uncommon',
      attachmentSlots: ['muzzle', 'underRail', 'sideRail', 'topRail', 'stock', 'grip', 'magazine'],
      // Spread retuned (hip 0.13 → 0.17, ads 0.028 → 0.040) per the
      // SMG-spread balance pass — base accuracy is now spray-tier;
      // attachments + skills are required for tight grouping.
      fireMode: 'auto',
      fireRate: 14,
      damage: 16,
      range: 26,
    ...
    // === Ricochet Revolver family (RX-series) ==========================
    // Bullet reflects off walls with a small random Y veer. Each bounce
    // loses a fraction of its damage. Designed for chaining around cover.
    {
      worldDrop: true,
      name: 'RX-1 Rebound', type: 'ranged', class: 'exotic', rarity: 'rare',
      minFloor: 2,
      fireMode: 'ricochetBounce',
      maxBounces: 2,
      bounceDmgMult: 0.85,
      yVeerRange: 0.4,
      damage: 65, range: 22, hipSpread: 0.04, adsSpread: 0.012,
      adsZoom: 0.95, adsPeekDistance: 4.5,
      tracerColor: 0xffd8a0, muzzleLength: 0.55, muzzleGirth: 0.18,
      pelletCount: 1, burstCount: 1, burstInterval: 0,
      magSize: 6, reloadTime: 1.8, fireRate: 0.42,
      attachmentSlots: ['topRail', 'sideRail'],
      description: 'Six-shot revolver. Rounds ricochet off walls up to 2 times with a slight vertical veer. 85% damage per bounce.',
    },
    {
      worldDrop: true,
      name: 'RX-2 Caroma', type: 'ranged', class: 'exotic', rarity: 'epic',
      minFloor: 3,
      fireMode: 'ricochetBounce',
      maxBounces: 3,
      bounceDmgMult: 0.80,
      yVeerRange: 0.8,
      damage: 50, range: 22, hipSpread: 0.05, adsSpread: 0.015,
      adsZoom: 0.95, adsPeekDistance: 4.5,
      tracerColor: 0xffc070, muzzleLength: 0.55, muzzleGirth: 0.20,
      pelletCount: 1, burstCount: 1, burstInterval: 0,
      magSize: 6, reloadTime: 2.0, fireRate: 0.40,
      attachmentSlots: ['topRail', 'sideRail'],
      description: 'Wider-spread RX. 3 ricochets, larger vertical veer — saturates a corridor. Lower direct damage, higher coverage.',
    },
    {
      worldDrop: true,
      name: 'RX-3 Pinball', type: 'ranged', class: 'exotic', rarity: 'legendary',
      minFloor: 5,
      fireMode: 'ricochetBounce',
      maxBounces: 4,
      bounceDmgMult: 0.90,
      yVeerRange: 0.5,
      damage: 80, range: 24, hipSpread: 0.03, adsSpread: 0.010,
      adsZoom: 1.05, adsPeekDistance: 5.0,
      tracerColor: 0xff8030, muzzleLength: 0.58, muzzleGirth: 0.20,
      pelletCount: 1, burstCount: 1, burstInterval: 0,
      magSize: 8, reloadTime: 1.6, fireRate: 0.32,
      attachmentSlots: ['topRail', 'sideRail', 'underRail'],
      description: 'Tuned ricochet platform. 4 bounces at 90% damage each, faster fire rate, tighter spread. A bullet can clear a room.',
    },

    // === Charge Cannon family (VC-series) ==============================
    // Hold-to-charge energy beam, 3 discrete tiers. Higher tier = more
    // ammo cost, more damage, thicker beam.
    {
      worldDrop: true,
      name: 'VC-7 Volt', type: 'ranged', class: 'exotic', rarity: 'rare',
      minFloor: 2,
      fireMode: 'charge',
      chargeTiers: [
        { thresholdSec: 0.0, dmgMult: 1.0, ammoCost: 1, beamWidth: 0.05 },
        { thresholdSec: 0.5, dmgMult: 2.0, ammoCost: 2, beamWidth: 0.10 },
        { thresholdSec: 1.2, dmgMult: 4.5, ammoCost: 4, beamWidth: 0.18 },
      ],
      damage: 40, range: 26, hipSpread: 0.0, adsSpread: 0.0,
      adsZoom: 0.85, adsPeekDistance: 4.8,
      tracerColor: 0x40d0ff, muzzleLength: 0.75, muzzleGirth: 0.22,
      pelletCount: 1, burstCount: 1, burstInterval: 0,
      magSize: 8, reloadTime: 2.6, fireRate: 0.05,
      attachmentSlots: ['topRail', 'sideRail', 'underRail'],
      description: 'Energy cannon. Hold fire to charge — release for a beam at one of 3 tiers. Higher tier costs more ammo but does dramatically more damage.',
    },
    {
      worldDrop: true,
      name: 'VC-9 Surge', type: 'ranged', class: 'exotic', rarity: 'epic',
      minFloor: 3,
      fireMode: 'charge',
      chargeTiers: [
        { thresholdSec: 0.0, dmgMult: 1.2, ammoCost: 1, beamWidth: 0.06 },
        { thresholdSec: 0.3, dmgMult: 2.3, ammoCost: 2, beamWidth: 0.12 },
        { thresholdSec: 0.9, dmgMult: 5.0, ammoCost: 3, beamWidth: 0.20 },
      ],
      damage: 45, range: 26, hipSpread: 0.0, adsSpread: 0.0,
      adsZoom: 0.90, adsPeekDistance: 4.8,
      tracerColor: 0x60ffd0, muzzleLength: 0.78, muzzleGirth: 0.22,
      pelletCount: 1, burstCount: 1, burstInterval: 0,
      magSize: 9, reloadTime: 2.4, fireRate: 0.05,
      attachmentSlots: ['topRail', 'sideRail', 'underRail'],
      description: 'Faster-charging VC. Tier 3 ready in 0.9s and costs only 3 ammo. Quick repeated bursts at the cost of peak damage scaling.',
    },
    {
      worldDrop: true,
      name: 'VC-12 Nova', type: 'ranged', class: 'exotic', rarity: 'legendary',
      minFloor: 5,
      fireMode: 'charge',
      chargeTiers: [
        { thresholdSec: 0.0, dmgMult: 1.5, ammoCost: 1, beamWidth: 0.08 },
        { thresholdSec: 0.6, dmgMult: 3.0, ammoCost: 3, beamWidth: 0.16 },
        { thresholdSec: 1.5, dmgMult: 7.0, ammoCost: 5, beamWidth: 0.26 },
      ],
      damage: 55, range: 30, hipSpread: 0.0, adsSpread: 0.0,
      adsZoom: 1.00, adsPeekDistance: 5.2,
      tracerColor: 0xff60ff, muzzleLength: 0.82, muzzleGirth: 0.26,
      pelletCount: 1, burstCount: 1, burstInterval: 0,
      magSize: 10, reloadTime: 2.8, fireRate: 0.05,
      attachmentSlots: ['topRail', 'sideRail', 'underRail', 'stock'],
      description: 'Apex VC. Tier 3 hits like a freight train and the beam is wide enough to clip multiple enemies. Long charge time is the cost.',
    },

    // === Gauss Rifle family (GR-series) ================================
    // Fast-fire kinetic. Each round pushes enemies; if knockback slams
    // them into a wall, bonus damage applies.
    {
      worldDrop: true,
      name: 'GR-4 Slug', type: 'ranged', class: 'exotic', rarity: 'rare',
      minFloor: 2,
      fireMode: 'gauss',
      knockbackStrength: 7.0,
      wallSlamBonusDmg: 35,
      damage: 8, range: 24, hipSpread: 0.025, adsSpread: 0.008,
      adsZoom: 1.10, adsPeekDistance: 4.6,
      tracerColor: 0xa0e0ff, muzzleLength: 0.92, muzzleGirth: 0.16,
      pelletCount: 1, burstCount: 1, burstInterval: 0,
      magSize: 30, reloadTime: 2.4, fireRate: 0.08,
      attachmentSlots: ['topRail', 'sideRail', 'underRail', 'stock'],
      description: 'Electromagnetic rifle. Very high fire rate. Each round pushes enemies back; slamming them into a wall does +35 damage.',
    },
    {
      worldDrop: true,
      name: 'GR-6 Coil', type: 'ranged', class: 'exotic', rarity: 'epic',
      minFloor: 3,
      fireMode: 'gauss',
      knockbackStrength: 9.5,
      wallSlamBonusDmg: 50,
      damage: 9, range: 24, hipSpread: 0.020, adsSpread: 0.006,
      adsZoom: 1.15, adsPeekDistance: 4.8,
      tracerColor: 0x60c0ff, muzzleLength: 0.95, muzzleGirth: 0.16,
      pelletCount: 1, burstCount: 1, burstInterval: 0,
      magSize: 36, reloadTime: 2.4, fireRate: 0.05,
      attachmentSlots: ['topRail', 'sideRail', 'underRail', 'stock'],
      description: 'Tuned-coil GR. Even faster fire rate, harder push, +50 wall-slam damage. Pin enemies into corners and watch them fold.',
    },
    {
      worldDrop: true,
      name: 'GR-9 Mag', type: 'ranged', class: 'exotic', rarity: 'legendary',
      minFloor: 5,
      fireMode: 'gauss',
      knockbackStrength: 12.0,
      wallSlamBonusDmg: 70,
      damage: 12, range: 26, hipSpread: 0.018, adsSpread: 0.005,
      adsZoom: 1.20, adsPeekDistance: 5.0,
      tracerColor: 0x4080ff, muzzleLength: 1.00, muzzleGirth: 0.18,
      pelletCount: 1, burstCount: 1, burstInterval: 0,
      magSize: 32, reloadTime: 2.6, fireRate: 0.06,
      attachmentSlots: ['topRail', 'sideRail', 'underRail', 'stock'],
      description: 'Heavy-bore GR. Each round hits harder AND pushes harder; wall slam is +70 damage. Crowd control that murders.',
    },

    // === Explosive Crossbow family (EX-series) =========================
    // Projectile sticks on impact, short fuse, AoE detonation. Enemy
    // it stuck to takes bonus damage on detonation.
    {
      worldDrop: true,
      name: 'EX-3 Bolt', type: 'ranged', class: 'exotic', rarity: 'rare',
      minFloor: 2,
      fireMode: 'stickyExplosive',
      fuseSec: 0.6,
      explosionRadius: 3.0,
      explosionDmg: 50,
      stuckBonusDmg: 35,
      projectileSpeed: 38,
      damage: 50, range: 24, hipSpread: 0.01, adsSpread: 0.0,
      adsZoom: 1.20, adsPeekDistance: 5.0,
      tracerColor: 0xff6020, muzzleLength: 0.95, muzzleGirth: 0.14,
      pelletCount: 1, burstCount: 1, burstInterval: 0,
      magSize: 4, reloadTime: 2.8, fireRate: 0.55,
      attachmentSlots: ['topRail', 'sideRail', 'stock'],
      description: 'Crossbow firing explosive bolts. Bolt sticks on impact, detonates 0.6s later (3m radius, 50 dmg). Stuck enemy eats an extra 35.',
    },
    {
      worldDrop: true,
      name: 'EX-5 Spike', type: 'ranged', class: 'exotic', rarity: 'epic',
      minFloor: 3,
      fireMode: 'stickyExplosive',
      fuseSec: 0.4,
      explosionRadius: 2.8,
      ...
      stuckBonusDmg: 50,
      projectileSpeed: 42,
      damage: 45, range: 24, hipSpread: 0.01, adsSpread: 0.0,
      adsZoom: 1.25, adsPeekDistance: 5.0,
      tracerColor: 0xff8040, muzzleLength: 0.95, muzzleGirth: 0.14,
      pelletCount: 1, burstCount: 1, burstInterval: 0,
      magSize: 5, reloadTime: 2.6, fireRate: 0.45,
      attachmentSlots: ['topRail', 'sideRail', 'stock'],
      description: 'Quick-fuse EX. 0.4s detonation, +50 bonus damage to the stuck enemy. Stick-and-step-back playstyle.',
    },
    {
      worldDrop: true,
      name: 'EX-8 Stinger', type: 'ranged', class: 'exotic', rarity: 'legendary',
      minFloor: 5,
      fireMode: 'stickyExplosive',
      fuseSec: 0.6,
      explosionRadius: 4.0,
      ...
      stuckBonusDmg: 60,
      projectileSpeed: 40,
      damage: 75, range: 26, hipSpread: 0.008, adsSpread: 0.0,
      adsZoom: 1.30, adsPeekDistance: 5.2,
      tracerColor: 0xff4010, muzzleLength: 1.00, muzzleGirth: 0.16,
      pelletCount: 1, burstCount: 1, burstInterval: 0,
      magSize: 4, reloadTime: 3.0, fireRate: 0.55,
      attachmentSlots: ['topRail', 'sideRail', 'underRail', 'stock'],
      description: 'Big-payload EX. 4m radius, 75 damage explosion, +60 stuck bonus. Murders clusters, makes bosses miserable.',
    },
  ],
